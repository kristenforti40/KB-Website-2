...
const TeamMemberCard: React.FC<{ member: TeamMember, index: number }> = ({ member, index }) => {
    ...
    return (
        <div className={`bg-white/70 backdrop-blur-sm rounded-lg shadow-xl overflow-hidden flex flex-col md:flex-row border border-stone-200/50 ${isReversed ? 'md:flex-row-reverse' : ''}`}>
            <div className="md:w-1/3 p-4">
                 <div className="bg-white p-2 rounded-md shadow-inner">
                    <img src={member.imageUrl} alt={member.name} className="object-cover h-full w-full rounded-sm" />
                </div>
            </div>
